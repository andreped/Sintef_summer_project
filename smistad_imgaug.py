...
class UltrasoundImageIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        #print('Making batch..')
        batch_x = np.zeros(tuple([len(index_array)] + list(self.input_shape)))
        ...
        for batch_index, sample_index in enumerate(index_array):
            ...
            batch_x[batch_index] = new_input_image
            batch_y[batch_index] = output
            # # TODO fix
            # # Multiple outputs
            # batch_y = []
            # for output_index in range(len(self.outputs)):
            #     batch_y.append(np.zeros(tuple([current_batch_size] + list(self.outputs[output_index].shape)[1:])))
            #
            # for batch_index, sample_index in enumerate(index_array):
            #     # Have to copy here in order to not modify original data
            #     input_image = self.inputs[sample_index].copy()
            #     output = [output[sample_index].copy() for output in self.outputs]
            #     new_input_image, output = self.generator.transform(input_image, output)
            #     batch_x[batch_index] = new_input_image
            #     for output_index in range(len(self.outputs)):
            #         batch_y[output_index][batch_index] = output[output_index]
